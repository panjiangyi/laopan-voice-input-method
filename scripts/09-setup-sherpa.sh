...
echo "[3/5] Downloading proven Paraformer fallback..."
mkdir -p "$ROOT/models"
if [ ! -d "$MODEL_DIR" ]; then
...
echo "[4/5] Paraformer fallback ready."

echo "[5/5] Installing bilingual hotword decoder..."
bash "$ROOT/scripts/21-setup-hotword-asr.sh"

echo "Done."
echo "Runtime backend: auto (Zipformer hotwords when ready; Paraformer fallback)"
